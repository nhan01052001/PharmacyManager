...
export const Login: React.FC = () => {
  ...
  const [
    {username, password, isError, isRememberPassword, isPasswordVisibility},
    setState,
  ...
  const handleOnChangeTextUsername = useCallback(
    (text: any) => {
      debugger;
      let nextState: any = {};
      if (text.length === 0) {
  ...
  const handleOnChangeTextPassword = useCallback(
    (text: any) => {
      debugger;
      let nextState: any = {};
      if (text.length === 0) {
        ...
        HttpService.Post(`${env.URL}/auth/login`, {
          ...
          .then((res: any) => {
            ...
            if (
              ...
              Function.setAppData(ENUM.KEY_IN4USER, {
                ...res?.data,
  ...
  useEffect(() => {
    ...
    GoogleSignin.configure({
      ...
      webClientId:
        '493934264080-edtlckbg0dlfqomsa7j1291oql9bhgks.apps.googleusercontent.com',
    });
  ...
  const onAuthStateChanged = (user: any) => {
    const dataUser = {
      ...
      birthday: '',
      address: '',
      id: Array.isArray(user?._user?.providerData)
        ? user?._user?.providerData[0]?.uid
        : user?._user?.uid
        ? user?._user?.uid
        : '',
    };
    ...
    try {
      HttpService.Get(`${env.URL}/user/getUserLoginSocial/${dataUser.id}`).then(
        (res: any) => {
          LoadingService.hide();
          if (res) {
            Function.setAppData(ENUM.KEY_IN4USER, {
            ...
            HttpService.Post(`${env.URL}/auth/registerWithSocial`, {
              ...dataUser,
            }).then((res: any) => {
        ...
        <View style={StylesTheme.paddingH24}>
          ...
          <View style={styles.wrapTextInput}>
            ...
            <View style={StylesTheme.marginV12}>
              <View
                style={[
                  StylesTheme.onlyFlexRow_AliCenter_JusSP,
                  {marginVertical: 2},
                ]}>
                <Text
                  numberOfLines={2}
              ...
              <Pressable
                ...
                <View
                  style={[
                    ...
                    {
                      backgroundColor: isRememberPassword
                        ? Colors.primaryColor
                        : '#fff',
                    },
                  ]}
          ...
          <View style={{flex: 1, justifyContent: 'center', marginTop: 24}}>
            ...
            <View style={[StylesTheme.flexRowCenter, StylesTheme.marginV24]}>
            ...
            <View style={StylesTheme.flexRowCenter}>
              {/* Login with google */}
              <View>
                <TouchableOpacity
            ...
            <View style={styles.wrapNotHaveAccount}>
