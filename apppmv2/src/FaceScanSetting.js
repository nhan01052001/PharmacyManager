...
import {RNCamera, FaceDetector} from 'react-native-camera'; // Import the necessary components
    ...
    try {
      ...
      if (camera && camera.takePictureAsync) {
        ...
        if (this.validateFaceAngles(faces?.faces, itemfind.index)) {
          ...
          AlertService.show(
            ENUM.E_ERROR,
            itemfind.lable + ' không thành công',
            5000,
          );
          this.isCapturing = false;
          this.setState({
        ...
        .then(res => {
          ...
          if (res?.status) {
            if (res?.status == 'success') {
              ...
              AlertService.show(
                ENUM.E_SUCCESS,
                'Thiết lập khuôn mặt thành công!',
                5000,
              );
              ...
              AlertService.show(
                ENUM.E_ERROR,
                'Không thể tìm thấy khuôn mặt trong ảnh đính kèm',
                5000,
              );
            }
          } else {
                          ...
                          <View
                            ...
                            <View style={styles.styheader}>
                              ...
                              <Text style={[styles.text, styles.subtitle]}>
                                Vui lòng giữ gương mặt ở giữa khung hình, di
                                chuyển khuôn mặt để camera bắt được nhiều góc
                                cạnh của gương mặt
                              </Text>
                            </View>

                            <CricleLoaderComponent
                            ...
                            {!isStarted ? (
                              <View style={styles.styBtnPrimary}>
                                ...
                                <TouchableOpacity
                                  ...
                                  <Text
                                    style={[styles.text, styles.notNowText]}>
                                    Lúc khác
                                  </Text>
                                </TouchableOpacity>
                              </View>
                                  ...
                                  {photos.map((item, index) => {
                                    return item.uri ? (
                                      <View
                                        ...
                                        />
                                        <Text
                                          style={[styles.text, styles.txtImg]}>
                                          {item.lable}
                                        </Text>
                                      </View>
                                    ) : (
